...
def encode_pinblock_iso_1(pin: str, transaction_field: _Optional[str] = None) -> bytes:
    r"""Encode ISO 9564 PIN block format 1 aka ANSI PIN block.
    ISO format 1 PIN block is an 8 byte value that consists of

        - Control field. A 4 bit hex value set to 1.
        - PIN length. A 4 bit hex value in the range from 4 to C.
        - PIN digits. Each digit is a 4 bit hex value in the range from 0 to 9.
        - Transaction digits. Each digit is a 4 bit hex value with permissible values of 0 to F.

    A filler is formed by 14-len(pin) transaction digits provided by the transaction field. It shall be unique for every
    occurrence of the PIN block and should not be transmitted with the PIN block.

    The transaction field can be derived from a transaction sequence, time stamp or similar and if so passed to the
    function as (optional) argument, otherwise a random value is generated by default.

    The format 1 PIN block should be only used in situations where the PAN is not available.

    Parameters
    ----------
    pin : str
        ASCII Personal Identification Number.
    transaction_field : _Optional[str]
        ASCII hex pseudorandom sequence of transaction digits.
    ...
    ValueError
        PIN must be between 4 and 12 digits long
        Transaction digits must be at least 'filler_len' hex digits long

    Examples
    --------
    >>> from psec.pinblock import encode_pinblock_iso_1
    >>> encode_pinblock_iso_1("1234").hex().upper()[:6]
    '141234'
    """
    ...
    filler_len = 14 - len(pin)
    if transaction_field is None:
        # Generate filler from random bytes
        transaction_field = _urandom(_ceil(filler_len / 2)).hex().upper()
    else:
        # Check format of provided transaction digits: min length, all hex digits '0' - 'F'
        if len(transaction_field) < filler_len or not all(
            c in _string.hexdigits for c in transaction_field
        ):
            raise ValueError(
                f"Transaction digits must be at least `{filler_len}` hex digits long"
            )

    filler = transaction_field[:filler_len]

    pin_len_hex = (
        len(pin).to_bytes(1, "big").hex()[1]
    )  # Only the low nibble is relevant, values 4 - C.

    return _binascii.a2b_hex("1" + pin_len_hex + pin + filler)
...
def decode_pinblock_iso_0(pinblock: bytes, pan: str) -> str:
    ...
    ValueError
        ...
        PIN length must be between 4 and 12: `pin_len`
...
def decode_pinblock_iso_1(pinblock: bytes) -> str:
    r"""Decode ISO 9564 PIN block format 1 aka ANSI PIN block.
    ISO format 1 PIN block is an 8 byte value that consists of
    ...
    ValueError
        PIN block must be 8 bytes long
        PIN block is not ISO format 1: control field `X`
        PIN is not numeric: `pin`
        PIN length must be between 4 and 12: `pin_len`"

    Examples
    --------
    >>> from psec.pinblock import decode_pinblock_iso_1
    >>> decode_pinblock_iso_1(bytes.fromhex("14123467F5965E2F"))
    '1234'
    """
    ...
    if block[0] != "1":
        raise ValueError(f"PIN block is not ISO format 1: control field `{block[0]}`")
